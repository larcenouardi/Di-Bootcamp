...
SELECT DISTINCT create_date FROM customer ORDER BY create_date ASC;
...
WHERE a.district = 'Texas' AND co.country = 'United States';
...
SELECT film_id, title, description, length, rental_rate FROM film WHERE title = 'Apollo Teen';
...
SELECT p.customer_id, c.first_name, c.last_name, p.amount, p.payment_date, s.staff_id
FROM payment p
